...
import { getMovimientosInventario, registrarAjuste } from '@/api/inventario'
...
async function loadData() {
  ...
  try {
    ...
    await showError(getFriendlyError(error, 'No fue posible cargar los ajustes.'))
  } finally {
    loading.value = false
  }
}

async function saveAdjustment(data: MovimientoFormData) {
  saving.value = true

  try {
    const response = await registrarAjuste({
    ...
    await showError(getFriendlyError(error, 'No fue posible registrar el ajuste.'))
  } finally {
    saving.value = false
  }
}
...
<template>
  <section>
    ...
    <div class="mt-4 mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">
          Registra el stock real después de revisar el producto.
        </p>
      </div>
    ...
    <BaseLoader v-if="loading" text="Cargando ajustes..." />
        ...
        <table class="mobile-stack-table w-full min-w-[900px] text-left text-sm">
          <thead class="border-b border-gray-200 bg-gray-50">
            <tr class="text-xs font-semibold uppercase text-gray-500">
              ...
              <th class="px-5 py-4">Stock real</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="item in filtered" :key="item.id" class="hover:bg-gray-50">
              ...
              <td data-label="Stock real" class="px-5 py-4">
                <StatusChip status="info" :label="String(item.cantidad)" />
              </td>
